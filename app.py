...
def json_response(payload: dict, sid: str, status: int = 200):
    ...
    response.set_cookie("sid", sid, max_age=60 * 60 * 24 * 30, samesite="Lax")
...
@app.get("/frontend.js")
def frontend():
    response = make_response((ROOT / "frontend.js").read_text(encoding="utf-8"))
...
def new_game():
    ...
    with _sessions_lock:
        _sessions[sid] = GameState()
        state = _sessions[sid]
    return json_response({"ok": True, "state": snapshot(state)}, sid)
...
def game_action(action: str):
    ...
    try:
        if action == "room":
            ok, result = state.visit_room(str(payload.get("room", "")))
        elif action == "storage":
            ok, result = state.solve_storage_riddle(str(payload.get("answer", "")))
        elif action == "pin":
            ok, result = state.attempt_pin(str(payload.get("guess", "")))
        elif action == "wordle":
            ok, result = state.submit_wordle(str(payload.get("guess", "")))
        elif action == "question":
            ok, result = state.ask_question(str(payload.get("character", "")))
        elif action == "accuse":
            ...
            return json_response({"ok": False, "result": "Unknown endpoint"}, sid, 404)
    except Exception as error:
        return json_response({"ok": False, "result": f"Server error: {error}"}, sid, 500)
    return json_response({"ok": ok, "result": result, "state": snapshot(state)}, sid)
