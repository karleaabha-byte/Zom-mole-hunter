...
class GameState:
    ...
    # Used to alternate Storage outcomes between new games.
    run_number = 0
    ...
    def __init__(self, seed=None):
        ...
        # ---------------------------------------------------------
        # ALTERNATING STORAGE OUTCOME FOR DEMONSTRATION
        #
        # Odd runs  -> NO Storage evidence
        # Even runs -> Storage evidence FOUND
        # ---------------------------------------------------------
    ...
    def solve_storage_riddle(self, answer):
        ...
        # Storage outcome is determined by the alternating run.
    ...
    def attempt_pin(self, guess):
        ...
        digits = "".join(
            ch for ch in str(guess) if ch.isdigit()
        )

        if digits != case.CORRECT_PIN:
            self._log(
                f"Incorrect PIN attempt #{self.pin_attempts}."
            )
        ...
        self._log(
            "PIN cracked. Restricted employee access unlocked."
        )

        # Zephyr decides whether to deploy the secondary
        # security challenge.
        self.security_challenge_active = (
            self.mole_ai.decide_security_sabotage(self)
        )

        self.security_challenge_complete = (
            not self.security_challenge_active
        )

        if self.security_challenge_active:
            self._log(
                "Zephyr deployed the secondary security verification."
            )
        else:
            self._log(
    ...
    def ask_question(self, character, question_key="alibi"):
        # Interrogation is intentionally available from the hub
        # without requiring the Cafeteria PIN or the secondary
        # security challenge.
        ...
        if character == case.MOLE:
            ...
            # Zephyr uses utility-based reasoning to choose
    ...
    def get_stats(self):
        return {
            ...
            "cafeteria_evidence_found":
                self.cafeteria_evidence_found,
            "security_challenge_complete":
                self.security_challenge_complete,
            "wordle_failed":
                self.wordle_failed,
            "wordle_attempts":
                list(self.wordle_attempts),
            "wordle_results":
                list(self.wordle_results),
            "mole_ai":
                self.mole_ai.stats(),
            "statements":
                self.evidence.suspect_statements,
            "notes":
                list(self.evidence.notes),
        }
